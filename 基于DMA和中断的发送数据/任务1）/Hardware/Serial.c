...
uint8_t Serial_RxPacket[4];  //用于接收数据包的缓存数组
...
void ReceiveData_Inquire()
{
	...
	if(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == SET)          //判断UASRT1的RXNE位是否为1
	{
		...
		OLED_ShowHexNum(1, 1, RX_data_Inquire, 2);                   //以1行、1列、长度为2的格式在OLED屏幕上显示RX_data_Inquire
	}
}
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == SET)  //判断UASRT1的RXNE位是否为1
	{
		...
		//若标志变量为0，则还没有开始接收数据，等待接收包头FF
		if(RxState == 0)
		{
			if(RxData == '@')
			{
				RxState = 1;
				pRxPacket = 0;   //为了保险，将计数用的标志变量清0
			}
			else
				RxState = 0;
		}
		...
		{
			//因为接收的数据数量不确定，每次接收时就要判断接收的是不是结束符
			if(RxData == '\r')
			{
				RxState = 2;
			}
		...
		else if(RxState == 2)
		{
			if(RxData == '\n')
			{
				RxState = 0;
				Serial_RxFlag = 1;  
				Serial_RxPacket[pRxPacket] = '\0';     //在字符串末尾添加一个\0，用于标记字符串在这里结束
			}
		}
...
void PA5_OFF()
{
	...
	GPIO_Initstructure_A.GPIO_Mode = GPIO_Mode_AIN; //设置GPIOA端口为推挽输出，点亮引脚PA5的灯
	GPIO_Initstructure_A.GPIO_Pin = GPIO_Pin_5;        //点亮引脚PA5的灯
	GPIO_Initstructure_A.GPIO_Speed = GPIO_Speed_50MHz;//速度为50MHz
